...
func Subscribe(token string) error {
	if fcmClient == nil {
		Init()
	}

	_, err := fcmClient.SubscribeToTopic(getContext(), []string{token}, topic)
	if err != nil {
		return err
	}
...
func Unsubscribe(token string) error {
	...
	_, err := fcmClient.UnsubscribeFromTopic(getContext(), []string{token}, topic)
	if err != nil {
		return err
	}
